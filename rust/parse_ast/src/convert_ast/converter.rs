...
impl<'a> AstConverter<'a> {
  ...
  fn store_jsx_expr_container(&mut self, jsx_expr_container: &JSXExprContainer) {
    let end_position = self.add_type_and_start(
      &TYPE_JSX_EXPR_CONTAINER_INLINED_EXPRESSION,
  ...
  fn store_jsx_text(&mut self, jsx_text: &JSXText) {
    let end_position = self.add_type_and_start(
      &TYPE_JSX_TEXT_INLINED_VALUE,
      &jsx_text.span,
      JSX_TEXT_RESERVED_BYTES,
      false,
    );
    // value
    self.convert_string(&jsx_text.value);
    // end
  ...
  fn store_jsx_opening_element(&mut self, jsx_opening_element: &JSXOpeningElement) {
    ...
    // flags
    let mut flags = if jsx_opening_element.self_closing {
    ...
    self.convert_item_list(&jsx_opening_element.attrs, |ast_converter, jsx_attribute| {
      ast_converter.store_jsx_attribute_or_spread(jsx_attribute);
      true
    });
    // end
  ...
  fn store_jsx_attribute(&mut self, jsx_attribute: &JSXAttr) {
    let end_position = self.add_type_and_start(
      &TYPE_JSX_ATTRIBUTE_INLINED_NAME,
      &jsx_attribute.span,
      JSX_ATTRIBUTE_RESERVED_BYTES,
      false,
    );
    // name
    self.store_jsx_attr_name(&jsx_attribute.name);
    // value
  ...
  fn convert_jsx_fragment(&mut self, jsx_fragment: &JSXFragment) {
    ...
    let end_position = self.add_type_and_start(
      &TYPE_JSX_FRAGMENT_INLINED_OPENING_FRAGMENT,
      &jsx_fragment.span,
    ...
    // children
    self.update_reference_position(end_position + JSX_FRAGMENT_CHILDREN_OFFSET);
    self.convert_item_list(&jsx_fragment.children, |ast_converter, jsx_fragment_child| {
